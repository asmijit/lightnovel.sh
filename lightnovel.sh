...
# Author:       Asmijit Saha (modified) , Newman Sanchez (https://github.com/lr-tech)
# Github repo:  https://github.com/asmijit/lightnovel.sh
...
white="\033[1;37m"
default="\033[0m"
# NOTE: User reported changing this URL from freewebnovel.com to innread.com
url='https://innread.com'
history="${cache_dir}/history"
history_lenght=20

# === MODIFICATION START ===
# Added a strong, modern User-Agent string to bypass anti-scraping checks.
...
get_chapter_count() {
    # === MODIFICATION: Added -A "${user_agent}" flag ===
    lastest_chapter_url=$(curl -s -A "${user_agent}" "${novel_url}" | grep 'lastest_chapter_url' |
        sed 's/^.*content="//;s/">//')
...
    # === MODIFICATION: Added -A "${user_agent}" flag ===
    scraped_results=$(curl -s -A "${user_agent}" -d "${params}" "${url}/search/")
...
    # === MODIFICATION: Added -A "${user_agent}" flag ===
    retrieved_chapter="$(curl -s -A "${user_agent}" "${chapter_url}" |
